 ...
 * 
 * @author Het Daftary
 * This file has the functions to read, write and generate Random CSV files a CSV file.
 * Here we use vectors as it is easier to work with vectors.
 */
...
vector<vector<string>> csv_reader_vector(string file_name, string delimiter) {
    ...
    if (csv_file.is_open()) 
        while (getline(csv_file, temp_line))
            data.push_back(split_String_vector(temp_line, delimiter));
    else 
...
void csv_writer_vector(vector<vector<string>> data, string delimiter, string file_name) {
    fstream csv_file;
    csv_file.open(file_name, ios::out);

    if (csv_file.is_open())
        for (vector<string> parts_of_str: data) 
            csv_file << join_Strings_vector(parts_of_str, delimiter);
    else
...
void generate_random_csv_vector(string file_name, string delimiter, int capacity, vector<Field> fields) {
    /**
     * 
    ...
    if (fptr.is_open()) {
        for (int i = 0; i < capacity; i++) {
            ...
            fptr << join_Strings_vector(to_write, delimiter) + "\n";
        }
    } else {
        cout << "Error" << endl;
    } fptr.close();
